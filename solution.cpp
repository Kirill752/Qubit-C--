...
int main(int argc, char *argv[])
{
   ...
   cout << "Матрица ёмкостных коэффициентов: " << endl;
   cout << cap_eigen << endl;
   ...
   cout << "Матрица электростатической индукции для двух капель: " << endl;
   cout << beta_dd << endl;

   // Вектор потенциалов на каплях
   ...
   // выводим решение в файл "charge_energy.txt"
   ofstream fout("charge_energy.txt");
     for (int n1 = -10; n1 < 11; ++n1)
   {
      for (int n2 = -10; n2 < 11; ++n2)
      {
         q_dd << n1, n2;
         fout << n1 << "   " << n2 << "   " << 0.5*q_dd.transpose()*beta_dd.inverse()*q_dd << endl;
      }
   }   
   fout.close();
